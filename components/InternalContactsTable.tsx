...
const InternalContactsTable: React.FC<InternalContactsTableProps> = ({ data, loading }) => {
  ...
  const departments = useMemo(() => {
    const depts = [...new Set(data.map(item => item.관리부서).filter(Boolean))];
    return depts.sort();
  }, [data]);
  ...
  const filteredData = useMemo(() => {
    ...
    // 부서 필터
    if (selectedDept !== 'all') {
      result = result.filter(item => item.관리부서 === selectedDept);
    }
    ...
    if (searchTerm) {
      ...
      result = result.filter(item =>
        item.품목명?.toLowerCase().includes(term) ||
        item.담당자?.toLowerCase().includes(term) ||
        item.관리부서?.toLowerCase().includes(term) ||
        ...
        <div className="flex flex-wrap justify-between items-center gap-3">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-zinc-200">Team Contacts</h3>
            </div>
            <div className="flex items-center gap-2 text-[10px] font-mono bg-cyan-500/10 text-cyan-400 px-2 py-1 rounded border border-cyan-500/20">
              <span>{data.length}명 담당자</span>
            </div>
          </div>
          <div className="text-xs text-zinc-500 font-mono">
            {filteredData.length} / {data.length} 연락처
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h4 className="font-semibold text-white text-sm">{contact.품목명}</h4>
                    <span className={`inline-block mt-1.5 text-[10px] px-2 py-0.5 rounded border ${getDeptColor(contact.관리부서)}`}>
                      {contact.관리부서}
                    </span>
                  </div>
                  ...
                  {contact.이메일 && (
                    <div className="flex items-center gap-2">
                      ...
                      <a 
                        ...
                        className="text-cyan-400 hover:text-cyan-300 text-xs truncate"
                      >
                        {contact.이메일}
                      </a>
                    </div>
